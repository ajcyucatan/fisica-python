			...
			do
				...
				case $input in
					[yY][eE][sS]|[yY])
						echo "yes"
						cd ~; ls
						printf "\e[31mMake sure your system satisfies the following dependencies for Conda:"
						printf "wget build-essential libgl1-mesa-glx libegl1-mesa libxrandr2 libxrandr2 libxss1 libxcursor1 libxcomposite1 libasound2 libxi6 libxtst6\033\e[0m"
						echo "Now we are going to install Conda..."
						MACHINE_TYPE=`uname -m`
						if [ ${MACHINE_TYPE} == 'x86_64' ]; then
							FILE=`mktemp`; sudo wget https://repo.continuum.io/miniconda/Miniconda3-latest-Linux-x86_64.sh -O ~/miniconda.sh
					...
					[nN][oO]|[nN])
						...
						if [ $(program_is_installed pip) == 1 ]; then
							...
							printf "\e[31mMake sure to install:"
							printf "build-essential python-dev python-pip\033\e[0m"
							sudo pip install --upgrade pip
							sudo pip install --upgrade virtualenv
							printf "Pip installed!"
							which pip
